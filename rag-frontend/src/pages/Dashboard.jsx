      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold  flex items-center gap-2 ">
            ...
             {" "}
            <FileBox
              className="text-brand-500 drop-shadow-[0_0_8px_rgba(59,130,246,0.6)]"
              size={26}
            />
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            {recentDocs.length > 0 ? (
              recentDocs.map((doc) => (
                <div
                  key={doc.id}
                  className="flex items-center justify-between p-3 rounded-xl bg-slate-200 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-800"
                >
        ...
        <div className="bg-white dark:bg-darkCard rounded-2xl border border-slate-200 dark:border-darkBorder p-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Link
              to="/chat"
              className="p-4 rounded-xl bg-gradient-to-br from-brand-50 to-indigo-500 dark:from-brand-950/30 dark:to-indigo-500 border border-brand-100 dark:border-brand-900/50 hover:shadow-md transition group"
            >
              <MessageSquareText
                size={24}
                className="text-brand-600 dark:text-brand-400 mb-2 group-hover:scale-110 transition-transform"
              />
              <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                Ask a Question
              </h3>
              <p className="text-xs text-slate-700 dark:text-slate-900 mt-1">
                Query your entire library
              </p>
            </Link>
            <Link
              to="/search"
              className="p-4 rounded-xl bg-gradient-to-br from-emerald-50 to-teal-500 dark:from-emerald-950/30 dark:to-teal-500 border border-emerald-100 dark:border-emerald-900/50 hover:shadow-md transition group"
            >
              <Search
                size={24}
                className="text-emerald-600 dark:text-emerald-400 mb-2 group-hover:scale-110 transition-transform"
              />
              <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                Deep Search
              </h3>
