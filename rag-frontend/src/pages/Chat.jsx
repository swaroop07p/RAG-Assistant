...
import { Plus, MessageSquare, Trash2, Edit2, Loader2, PanelLeftClose, PanelLeftOpen } from 'lucide-react';
...
export const Chat = () => {
  ...
  const handleCitationClick = (citation) => {
    const matchedDoc = documents.find(d => d.id === citation.document_id || d.original_name === citation.document_name);
    if (matchedDoc) {
      setActiveCitation({
        doc: matchedDoc,
  ...
  const handleSendMessage = async (text) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'ai', content: '**Error:** Failed to generate response. Please try again.', isError: true, id: `err_${Date.now()}` }]);
    } finally {
  ...
  return (
    <div className="flex md:h-[calc(100vh-8rem)] h-[calc(100vh-6rem)] bg-white dark:bg-darkCard rounded-2xl border border-slate-200 dark:border-darkBorder overflow-hidden shadow-sm">
          ...
          <>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-1">
                <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider px-2 py-2">Recent Chats</p>
                {sessions.map(session => (
                  ...
                  >
                    <div className="flex items-center gap-2 overflow-hidden">
                      <MessageSquare size={16} className={currentSessionId === session.id ? 'text-brand-500' : 'text-slate-400'} />
                      <span className="text-sm text-slate-700 dark:text-slate-300 truncate font-medium">{session.title}</span>
                    </div>
                    <button
      ...
      <div className="flex-1 flex flex-col min-w-0 relative">
        ...
        <div className="h-14 border-b border-slate-200 dark:border-darkBorder bg-white/80 dark:bg-darkCard/80 backdrop-blur-sm flex items-center justify-between px-4 sticky top-0 z-10 shrink-0">
          ...
          >
            {selectedDocIds.length === 0 ? 'Searching: All Documents' : `Searching: ${selectedDocIds.length} Document(s)`}
          </button>
        </div>
          ...
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 mb-8">
                Ask a question and I'll search through your enterprise documents to find the exact answer, complete with page citations.
              </p>
            </div>
          ) : (
            messages.map((msg) => (
        ...
        <div className="p-4 bg-white dark:bg-darkCard border-t border-slate-200 dark:border-darkBorder shrink-0">
          <ChatInput onSend={handleSendMessage} disabled={isGenerating} onOpenDocSelector={() => setIsDocSelectorOpen(true)} />
        </div>
      </div>
