...
export const Chat = () => {
  ...
  const [selectedDocIds, setSelectedDocIds] = useState([]); // Empty array means 'Search All'
  ...
  const handleSendMessage = async (text) => {
    ...
    try {
      ...
      if (!currentSessionId) {
        ...
        fetchSessions(); // Refresh list to get new session
      }
        ...
        {isHistoryOpen && (
          <>
            ...
            <motion.div
              ...
              className="absolute md:relative z-30 h-full border-r border-slate-200 dark:border-darkBorder bg-slate-50 dark:bg-slate-900 flex flex-col shrink-0 overflow-hidden shadow-xl md:shadow-none"
            >
              {/* ... keep the inner sidebar content exactly the same ... */}
